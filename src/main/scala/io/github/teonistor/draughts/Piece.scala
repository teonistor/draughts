...
sealed trait Piece {
  def emitMoves(from: Position): LazyList[Move]
  def promote: Piece
}
...
object Piece {
  ...
  private class King extends Piece {
    override def emitMoves(from: Position): LazyList[Move] =
      `+-1`.flatMap(dx =>
        `+-1`.flatMap(dy =>
          emitMovesInOneDirection(from, dx+_, dy+_)))
  ...
  private class Peon(forward: Int=>Int, val promote: Piece) extends Piece {
    override def emitMoves(from: Position): LazyList[Move] =
      `+-1`.flatMap(dx =>
        emitMovesInOneDirection(from, dx+_, forward))
  }

  private def emitMovesInOneDirection(from: Position, dx: Int=>Int, dy: Int=>Int): List[Move] = {
    val x1 = dx(from.x)
    val y1 = dy(from.y)
    val p1 = Position(x1, y1)
    List(
      Move.Sliding(from, p1),
      Move.Jumping(from, p1, Position(dx(x1), dy(y1))))
  }
}
