...
class EnPassantTest extends MoveTest{
    ...
    @Test
    void validateValid() {
        assertThat(new EnPassant(G4, F3, F4, F2).validate(new GameState(HashMap.of(F4, new Pawn(White)), Black, List.empty(),
                stateWith(HashMap.of(F2, new Pawn(White)), Black))))
                .isTrue();
    }
    ...
    void validateInvalid() {
        ...
        assertThat(new EnPassant(G4, H3, H4, H2).validate(new GameState(HashMap.of(H4, new Pawn(White)), Black, List.empty(),
                stateWith(HashMap.of(H3, new Pawn(White)), Black)))).isFalse();

        //It's not where it should be now
        assertThat(new EnPassant(E4, D3, D4, D2).validate(new GameState(HashMap.of(D3, new Pawn(White)), Black, List.empty(),
                stateWith(HashMap.of(D2, new Pawn(White)), Black)))).isFalse();

        // Wrong color (though the sequence doesn't really make sense
        assertThat(new EnPassant(G4, F3, F4, F2).validate(new GameState(HashMap.of(F4, new Pawn(Black)), Black, List.empty(),
                stateWith(HashMap.of(F2, new Pawn(Black)), Black)))).isFalse();
